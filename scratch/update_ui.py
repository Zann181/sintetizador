...
new_html_content = r"""    void serveHTML(int client_fd) {
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* All labels, values and headers are pure bright neutral white */
        ...
        h1 {
            ...
            letter-spacing: 2px; color: var(--neutral-white) !important;
            text-shadow: 0 0 6px var(--accent-glow);
            text-transform: uppercase;
        }
        .subtitle { color: var(--neutral-white) !important; opacity: 1 !important; font-size: 0.65rem; font-weight: 400; margin-top: 2px; text-transform: uppercase; letter-spacing: 0.5px; }
...
<body>
    <header>
        <div class="header-title-group">
            <h1>TR-808 Algorithmic Station</h1>
            <span class="subtitle">Motor Faust · RtAudio · OSC · Modulación de Parámetros</span>
        </div>
    ...
    <script>
        ...
        async function fetchParams() {
            try {
                ...
                // Sincronizar todos los controles del Mixer Console
                INSTRUMENTS.forEach(inst => {
                    const notePath = inst.key === 'siringe' ? inst.prefix + 'nota_base' : inst.prefix + 'nota';
                    ...
                    const notaParam = parameters.find(p => p.path === notePath);
                    if (notaParam) {
            ...
            container.innerHTML = `
                ...
                <div class="mixer-channels-grid">
                    ${INSTRUMENTS.map(inst => {
                        const notePath = inst.key === 'siringe' ? inst.prefix + 'nota_base' : inst.prefix + 'nota';
                        const noteParam = parameters.find(p => p.path === notePath) || { value: 36 };
                        const grooveParam = parameters.find(p => p.path === inst.prefix + 'groove') || { value: 0 };
                        ...
                        return `
                            <div class="mixer-channel-strip" id="chan-${inst.key}">
                                ...
                                <!-- Accent & Swing Knobs (Side-by-Side) -->
                                ...
                                <!-- Groove & Nota Selectors -->
                                <div class="mixer-widget-box">
                                    <span class="mixer-lbl">Tonalidad</span>
                                    <select class="mixer-select" id="select-inst-master-nota-${inst.key}" onchange="updateParam('${notePath}', this.value)">
                                ...
                                <div class="mixer-widget-box">
                                    ...
                                    <input type="range" class="mixer-slider-h" min="${decayParam.min}" max="${decayParam.max}" step="0.01" value="${decayParam.value}"
                                ...
                                <div class="fader-widget-vertical">
                                    ...
                                    <div class="fader-track-container">
                                        <input type="range" class="fader-slider-v" min="${volParam.min}" max="${volParam.max}" step="0.01" value="${volParam.value}"
        ...
        function dragKnob(e) {
            ...
            updateParam(activeKnob.getAttribute('data-path'), nv);
...
if start_idx == -1 or end_idx == -1:
    print("Error: Could not find start or end tags in OscServer.cpp!")
    exit(1)

new_content = content[:start_idx] + new_html_content + "\n" + content[end_idx:]

with open(file_path, "w", encoding="utf-8") as f:
    f.write(new_content)

print("OscServer.cpp served HTML code updated successfully with custom warm colors and LCD in header!")
