...
struct CopyableAtomicBool {
	...
	CopyableAtomicBool(bool b) : val(b) {}
	CopyableAtomicBool(const CopyableAtomicBool& other) : val(other.val.load(std::memory_order_acquire)) {}
...
public:
	int m_lastStep = -1;
	uint64_t m_lastStep0TimeMs = 0;
	uint64_t m_lastBeatTimeMs = 0;
	int m_lastBeatStep = -1;

	uint64_t m_masterSampleCounter = 0;
	float m_masterBpmVal = 140.0f;
	int m_lastBeatStepCalculated = -1;
	CopyableAtomicBool m_resetRequested{false};

	virtual int getCurrentStep() override {
		return m_lastStep;
	}
	virtual uint64_t getLastStep0TimeMs() override {
		return m_lastStep0TimeMs;
	}
	virtual uint64_t getLastBeatTimeMs() override {
		return m_lastBeatTimeMs;
	}
	virtual int getLastBeatStep() override {
		return m_lastBeatStep;
	}
	virtual void setBpm(float bpm) override {
		m_masterBpmVal = bpm;
	}
	virtual void resetSequencer() override {
		m_resetRequested.val.store(true, std::memory_order_release);
	}
 ...
 public:
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		uint64_t nowMs = std::chrono::duration_cast<std::chrono::milliseconds>(duration).count();

		if (m_resetRequested.val.exchange(false, std::memory_order_acq_rel)) {
			m_masterSampleCounter = 0;
			m_lastBeatStepCalculated = -1;
		}
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			m_masterSampleCounter++;
			double samplesPerStep = (fSampleRate * 15.0) / static_cast<double>(m_masterBpmVal);
			int currentStep = static_cast<int>(m_masterSampleCounter / samplesPerStep) % 64;
			if (currentStep != m_lastBeatStepCalculated) {
				m_lastBeatStepCalculated = currentStep;
				if (currentStep % 4 == 0) {
					double offsetMs = (static_cast<double>(i0) / fSampleRate) * 1000.0;
					uint64_t triggerTimeMs = nowMs + static_cast<uint64_t>(offsetMs);
					m_lastBeatTimeMs = triggerTimeMs;
					m_lastBeatStep = currentStep;
					if (currentStep == 0) {
						m_lastStep0TimeMs = triggerTimeMs;
					}
				}
			}
			m_lastStep = currentStep % 16;
